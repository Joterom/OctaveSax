function rev_algoritmo(alfa, beta, sample)
...
len = length(L);
f = Fs*(0:(len/2))/len;
index = floor(len/sample);
remin = rem(len, sample);
...
final_L = zeros(size(L));
final_R = zeros(size(R));

for i = 1:index
    % OJO, aqui es donde esta el problema
    cut_L = L((index-1)*sample+1:index*sample,1);
    cut_R = R((index-1)*sample+1:index*sample,1);
    ...
    % Left Channel
    ampL = abs(cut_L);
    phaL = unwrap(angle(cut_L));

    ampL_pos = ampL(2:sample/2+1);
    ampL_neg = ampL(sample/2+2:end);
    phaL_pos = phaL(2:sample/2+1);
    phaL_neg = phaL(sample/2+2:end);
    ...
    % Right Channel
    ampR = abs(cut_L);
    phaR = unwrap(angle(cut_L));

    ampR_pos = ampR(2:sample/2+1);
    ampR_neg = ampR(sample/2+2:end);
    phaR_pos = phaR(2:sample/2+1);
    phaR_neg = phaR(sample/2+2:end);
    ...
    final_R_fft((index-1)*sample+1:index*sample,1) = outR_fft;
    final_L_fft((index-1)*sample+1:index*sample,1) = outL_fft;

end

final_L = real(ifft(final_L_fft));
final_R = real(ifft(final_R_fft));

% Y su correspondiente grafica
PLaux = abs(final_L_fft/len);
PL = PLaux(1:len/2+1);
PL(2:end-1) = 2*PL(2:end-1);

PRaux = abs(final_R_fft/len);
PR = PRaux(1:len/2+1);
PR(2:end-1) = 2*PR(2:end-1);
figure(f3);
hold on;
