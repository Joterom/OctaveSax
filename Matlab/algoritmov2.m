function algoritmov2(alfa, beta)
...
% Y_L(1:10)
cont_Y_L = Y_L(1);
pos_Y_L = zeros(size(Y_L(2:length(Y_L)/2+1)));
pos_Y_L = Y_L(2:length(Y_L)/2+1);
pos_Y_L_compress = zeros(size(pos_Y_L));
for i=1:length(pos_Y_L)/2
    pos_Y_L_compress(i)=pos_Y_L(2*i);
end
% pos_Y_L(1:10)
% pos_Y_L_compress(1:5)
Y_L_compress = zeros(size(Y_L));
Y_L_compress(1)= cont_Y_L;
Y_L_compress(2:length(Y_L)/2+1)=pos_Y_L_compress;
Y_L_compress(length(Y_L)/2+2:end)=fliplr(pos_Y_L_compress(1:end-1)')';
Y_L_compress(1:10)
Y_L_compress(end-10:end)
